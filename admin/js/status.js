...
const HEADER_CACHE_KEY = "c13HeaderStatusCacheV1";
...
let hasCachedOpeningStatus = false;

function readHeaderCache() {
  try {
    const cached = JSON.parse(localStorage.getItem(HEADER_CACHE_KEY) || "null");
...
function updateHeaderCache(values) {
  try {
    const cached = readHeaderCache();
    localStorage.setItem(
      HEADER_CACHE_KEY,
      JSON.stringify({ ...cached, ...values, updatedAt: Date.now() })
    );
  } catch (_) {
    // Statusraden fungerar även om lokal lagring inte är tillgänglig.
  }
}
...
function renderCachedHeaderStatus() {
  const cached = readHeaderCache();

  if (cached.specialHours && typeof cached.specialHours === "object") {
    savedSpecialHours = cached.specialHours;
  }

  if (cached.openingHours && typeof cached.openingHours === "object") {
    savedOpeningHours = cached.openingHours;
    hasCachedOpeningStatus = true;
    renderOpeningStatus(savedOpeningHours);
  }

  if (cached.informationBar && typeof cached.informationBar === "object") {
    renderInformationBar(cached.informationBar);
  }

  if (Number.isFinite(cached.newArrivalsCount)) {
    renderNewArrivalsNotice(cached.newArrivalsCount);
  }
}
...
async function loadNewArrivalsNotice() {
  ...
  try {
    ...
    updateHeaderCache({ newArrivalsCount: count });
  } catch (error) {
    console.error("Kunde inte hämta notisen om nya plagg:", error);
    if (!Number.isFinite(readHeaderCache().newArrivalsCount)) {
...
async function loadStatusBar() {
  try {
    ...
    updateHeaderCache({
      ...
      specialHours: savedSpecialHours,
      informationBar: informationSnapshot.exists() ? informationSnapshot.data() : {}
    });
  } catch (error) {
    console.error("Kunde inte hämta statusraden:", error);
    if (!hasCachedOpeningStatus) {
