...
function render(root,ctx){
 ...
 if(r?.status==='complete'){
   ...
   body=`<div class="forward-lock"><b>G6 RESULTAT · ${r.gate.verdict}</b><span>Plan ${PLAN_HASH} · kandidat 15efd75a · boundary ${b==null?'ingen testpunkt':pct(b)+' extra/sida'}</span></div><h2>Kostnadskurva</h2>${r.points.map((s,i)=>`<div class="statusline"><b>${i+1}. Extra ${pct(s.extraCostPerSide)}</b> · total ${pct(s.totalCostPerSide)} · ${s.n} affärer · P/L ${fmt(s.pl)} · PF ${fmt(s.pf)} · WR ${pct(s.wr)} · DD ${pct(s.dd)} · slut ${fmt(s.final)}</div>`).join('')}<div class="statusline"><b>Gate ${r.gate.verdict}:</b> högsta testade gräns med P/L &gt; 0 och PF ≥ 1,00 = ${b==null?'ingen':pct(b)+' extra per sida'}.</div><div class="actions"><button id="g6Report" class="secondary">📥 Exportera G6-rapport</button><button id="g6Raw" class="secondary">Raw JSON</button></div>`;
 } else {
