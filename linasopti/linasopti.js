...
const V0413_SIM_SEED=19742; // dokumenterat minimum t.o.m. färdig Signal Lab 2
...
window.addEventListener('DOMContentLoaded',()=>{
 ...
 if(jump){const saved=localStorage.getItem(LAB_KEY);if(saved&&saved!=='v0458Lab'&&saved!=='v0459Lab'&&[...jump.options].some(o=>o.value===saved))jump.value=saved;else jump.value='v04511Lab';jump.addEventListener('change',showSelectedLab);showSelectedLab();}
...
async function v04511Share(){const st=document.getElementById('v04511Status');if(!V04511_RESULT){if(st)st.textContent='Ingen Signal Lab 3-rapport finns ännu. Kör labbet först.';return false}const text=v04511Report(),date=new Date().toISOString().slice(0,10),name=`LINAS_OPTI_SIGNAL_LAB_3_CLOSE_TESTET_V04512_${date}.txt`;if(!v0451IsIOS()){try{v0451DownloadText(text,name);if(st)st.textContent=`✓ Signal Lab 3-rapport nedladdad: ${name}`;return true}catch(e){if(st)st.textContent='Kunde inte ladda ner Signal Lab 3-rapporten.';return false}}const file=new File([text],name,{type:'text/plain;charset=utf-8'});try{if(navigator.share){if(!navigator.canShare||navigator.canShare({files:[file]})){await navigator.share({title:'Linas Opti Signal Lab 3',files:[file]});return true}await navigator.share({title:'Linas Opti Signal Lab 3',text});return true}}catch(e){if(e?.name==='AbortError')return false}try{v0451DownloadText(text,name);return true}catch{return false}}
