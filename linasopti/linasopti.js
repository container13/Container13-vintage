...
function v0430Freeze(){const R=v0430Rank(),eligible=R.filter(x=>x.eligible),picked=eligible.slice(0,16);V0430_FROZEN={version:'V0.43.3',created:new Date().toISOString(),rule:'n>=5, P/L>0, PF>=1, minst 3 positiva utvecklingsperioder; rank score för max 16',symbols:picked.map(x=>x.sym),stats:picked};localStorage.setItem(V0430_FREEZE_KEY,JSON.stringify(V0430_FROZEN));}
...
async function v043xShare(text,prefix,title){
 const name=`${prefix}_${new Date().toISOString().slice(0,10)}.txt`;
 const file=new File([text],name,{type:'text/plain'});
 const status=(msg)=>{const el=document.getElementById(prefix.includes('dayselection1')?'v0430Status':'v0431Status');if(el)el.textContent=msg};
 try{
  if(navigator.share){
   if(!navigator.canShare||navigator.canShare({files:[file]})){
    await navigator.share({title,text:'Linas Opti rapport',files:[file]});
    status('✓ Rapport delad.');
    return true;
   }
   await navigator.share({title,text});
   status('✓ Rapport delad som text.');
   return true;
  }
 }catch(e){
  if(e?.name==='AbortError'){status('Delning avbruten.');return false}
 }
 try{
  if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(text);status('✓ Rapport kopierad till urklipp.');return true}
 }catch(e){}
 try{
  const blob=new Blob([text],{type:'text/plain;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');
  a.href=url;a.download=name;a.style.display='none';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),3000);status('✓ Rapport sparad som textfil.');return true
