(function(){
'use strict';
const API='https://linas-opti-api.mangaj73.workers.dev', KEY='lina_clean_evidence_queue_v1', DIAG='lina_clean_sync_diagnostics_v0270', RELEASE='V0.2.80';
...
function syncStatus(){
 ...
 return {total:xs.length,pending:xs.filter(v=>v.status==='FROZEN'&&v.content).length,github:xs.filter(v=>v.status==='FROZEN · GITHUB ✓').length,preliminary:xs.filter(v=>v.status==='PRELIMINÄR').length,frozenWithoutContent:xs.filter(v=>v.status==='FROZEN'&&!v.content).length,gen5:xs.filter(v=>String(v.name||'').startsWith('LINAS_GEN5_')).map(v=>({name:v.name,status:v.status,source:v.source,githubPath:v.githubPath||null,verification:v.verification||null}))};
}
function reconcileAll(){const q=load();for(const z of q.items.filter(v=>v.status==='FROZEN · GITHUB ✓')){reconcileGen4Evidence(z);reconcileGen5Evidence(z)}return syncStatus()}
