(function(){
'use strict';
const API='https://linas-opti-api.mangaj73.workers.dev', KEY='lina_clean_evidence_queue_v1', DIAG='lina_clean_sync_diagnostics_v0270', RELEASE=window.LinaVersion?.release||'VERSION_UNAVAILABLE';
let syncFlight=null;
...
function diagLoad(){try{const d=JSON.parse(localStorage.getItem(DIAG)||'null');if(d){d.release=RELEASE;return d}}catch{}return{schema:'LINA-SYNC-DIAGNOSTICS-2',release:RELEASE,events:[]}}
...
function reconcileGen5Evidence(item){
 try{
  ...
  for(const f of Object.keys(x.gen5?.familyResults||{})){
   const ev=x.gen5.familyResults[f]?.evidence;
   const safe=f.toUpperCase().replace(/[^A-Z0-9]+/g,'_').replace(/^_|_$/g,'');
   const familyMatch=ev?.name===item.name||new RegExp('^LINAS_GEN5_'+safe+'_WALK_FORWARD_').test(item.name);
   if(familyMatch&&item.status==='FROZEN · GITHUB ✓'){
    x.gen5.familyResults[f].evidence={...(ev||{}),status:'FROZEN · GITHUB ✓',name:item.name,githubPath:item.githubPath||null,githubCommit:item.githubCommit||null,existingImmutable:Boolean(item.existingImmutable),verification:item.verification||null};changed=true
   }
  }
...
function reconcileGen6Evidence(item){
 try{
  const key='lina_generation_engine_v0273',raw=localStorage.getItem(key);if(!raw||!item?.name||item.status!=='FROZEN · GITHUB ✓')return;
  const x=JSON.parse(raw);if(!x.gen6)return;let changed=false;
  for(const f of Object.keys(x.gen6.familyResults||{})){
   const ev=x.gen6.familyResults[f]?.evidence;
   const safe=f.toUpperCase().replace(/[^A-Z0-9]+/g,'_').replace(/^_|_$/g,'');
   const familyMatch=ev?.name===item.name||new RegExp('^LINAS_GEN6_'+safe+'_WALK_FORWARD_').test(item.name);
   if(familyMatch){x.gen6.familyResults[f].evidence={...(ev||{}),status:'FROZEN · GITHUB ✓',name:item.name,githubPath:item.githubPath||null,githubCommit:item.githubCommit||null,existingImmutable:Boolean(item.existingImmutable),verification:item.verification||null};changed=true}
  }
  if(x.gen6.summaryFreeze?.frozen&&/^LINAS_GEN6_RESEARCH_SUMMARY_/.test(item.name)){
   x.gen6.summaryFreeze.evidence={...(x.gen6.summaryFreeze.evidence||{}),status:'FROZEN · GITHUB ✓',name:item.name,githubPath:item.githubPath||null,githubCommit:item.githubCommit||null,existingImmutable:Boolean(item.existingImmutable),verification:item.verification||null};changed=true;
  }
...
function reconcileGen7Evidence(item){
 try{
  const key='lina_generation_engine_v0273',raw=localStorage.getItem(key);if(!raw||!item?.name||item.status!=='FROZEN · GITHUB ✓')return;
  const x=JSON.parse(raw);if(!x.gen7)return;let changed=false;
  for(const f of Object.keys(x.gen7.familyResults||{})){
   const ev=x.gen7.familyResults[f]?.evidence,safe=f.toUpperCase().replace(/[^A-Z0-9]+/g,'_').replace(/^_|_$/g,'');
   if(ev?.name===item.name||new RegExp('^LINAS_GEN7_'+safe+'_WALK_FORWARD_').test(item.name)){x.gen7.familyResults[f].evidence={...(ev||{}),status:'FROZEN · GITHUB ✓',name:item.name,githubPath:item.githubPath||null,githubCommit:item.githubCommit||null,existingImmutable:Boolean(item.existingImmutable),verification:item.verification||null};changed=true}
  }
...
async function syncApprovedInner(){let q=load(),code=sessionStorage.getItem('linasopti_login_code')||'',n=0;if(!code){diag('evidence-blocked',{reason:'SESSION_MISSING'});throw new Error('Lina-session saknas – logga in igen')}const pending=q.items.filter(v=>v.status==='FROZEN'&&v.content);diag('evidence-sync-start',{pending:pending.map(x=>({name:x.name,source:x.source,mime:x.mime,contentBytes:new Blob([x.content]).size,sha256:x.sha256||null}))});for(const x of pending){const payload={schema:'LINA-EVIDENCE-1',release:RELEASE,status:'FROZEN',name:x.name,mime:x.mime,source:x.source,approvedAt:x.approvedAt,sha256:x.sha256,content:x.content}, body=JSON.stringify(payload);let r,j={},raw='';try{diag('evidence-request',{method:'POST',endpoint:API+'/evidence',name:x.name,source:x.source,contentBytes:new Blob([x.content]).size,bodyBytes:new Blob([body]).size,sha256:x.sha256||null});r=await fetch(API+'/evidence',{method:'POST',headers:{'Content-Type':'application/json','X-Lina-Login-Code':code},body});raw=await r.text();try{j=raw?JSON.parse(raw):{}}catch{j={}}diag('evidence-response',{name:x.name,httpStatus:r.status,httpStatusText:r.statusText||'',ok:r.ok,apiOk:Boolean(j.ok),response:raw.slice(0,4000),path:j.path||null,commit:j.commit||null});const exists409=r.status===409&&String(j?.error||'')==='Evidencefilen finns redan – original skrivs inte över';if((!r.ok||!j.ok)&&!exists409)throw new Error(j.error||('Evidence HTTP '+r.status));const date=/^\d{4}-\d{2}-\d{2}/.test(String(x.approvedAt||''))?String(x.approvedAt).slice(0,10):now().slice(0,10);x.status='FROZEN · GITHUB ✓';x.githubPath=j.path||`linasopti/evidence/${date}/${x.name}`;x.githubCommit=j.commit||null;x.syncedAt=now();if(exists409){x.existingImmutable=true;x.existenceVerifiedAt=x.syncedAt;x.verification='SERVER_CONFIRMED_IMMUTABLE_EXISTS_409';diag('evidence-existing-accepted',{name:x.name,httpStatus:409,path:x.githubPath,localSha256:x.sha256||null,note:'Servern bekräftade att immutable evidence redan finns. Ingen overwrite gjordes; innehållslikhet verifieras inte av nuvarande API.'})}delete x.content;n++;save(q);reconcileGen4Evidence(x);reconcileGen5Evidence(x);reconcileGen6Evidence(x);reconcileGen7Evidence(x)}catch(e){diag('evidence-error',{name:x.name,httpStatus:r?.status??null,httpStatusText:r?.statusText||null,message:String(e?.message||e),response:raw.slice(0,4000)});throw e}}for(const z of q.items.filter(v=>v.status==='FROZEN · GITHUB ✓')){reconcileGen4Evidence(z);reconcileGen5Evidence(z);reconcileGen6Evidence(z);reconcileGen7Evidence(z)}diag('evidence-sync-complete',{synced:n});return {synced:n,queue:syncStatus()}}
async function syncApproved(){if(syncFlight)return syncFlight;syncFlight=syncApprovedInner();try{return await syncFlight}finally{syncFlight=null}}

function syncStatus(){
 const q=load(),xs=q.items||[];
 return {total:xs.length,pending:xs.filter(v=>v.status==='FROZEN'&&v.content).length,github:xs.filter(v=>v.status==='FROZEN · GITHUB ✓').length,preliminary:xs.filter(v=>v.status==='PRELIMINÄR').length,frozenWithoutContent:xs.filter(v=>v.status==='FROZEN'&&!v.content).length,gen5:xs.filter(v=>String(v.name||'').startsWith('LINAS_GEN5_')).map(v=>({name:v.name,status:v.status,source:v.source,githubPath:v.githubPath||null,verification:v.verification||null})),gen6:xs.filter(v=>String(v.name||'').startsWith('LINAS_GEN6_')).map(v=>({name:v.name,status:v.status,source:v.source,githubPath:v.githubPath||null,verification:v.verification||null})),gen7:xs.filter(v=>String(v.name||'').startsWith('LINAS_GEN7_')).map(v=>({name:v.name,status:v.status,source:v.source,githubPath:v.githubPath||null,verification:v.verification||null}))};
}
function reconcileAll(){const q=load();for(const z of q.items.filter(v=>v.status==='FROZEN · GITHUB ✓')){reconcileGen4Evidence(z);reconcileGen5Evidence(z);reconcileGen6Evidence(z);reconcileGen7Evidence(z)}return syncStatus()}
...
install();document.addEventListener('lina:unlocked',install);window.LinaEvidence={stage,approve,approveAndSync,freezeFiles,freezeEngine,syncApproved,syncStatus,reconcileAll,items,sourceStatus,diagnostics,install};
})();
