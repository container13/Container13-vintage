(function(){
  ...
  function robustReport(){
    ...
    const L=['LINAS OPTI – G2 ROBUSTHETSANALYS','Clean Core: V0.2.15','Kandidat: '+r.candidateHash,'Handel: AVSTÄNGD','',
  ...
  function exportReport(){download(report(),`LINAS_OPTI_SWING_G2_A_O_CLEAN_V0211_${new Date().toISOString().slice(0,10)}.txt`)}
  function exportRaw(){const x=load();if(x)download(JSON.stringify(x,null,2),`LINAS_OPTI_SWING_G2_RAW_CLEAN_V0211_${new Date().toISOString().slice(0,10)}.json`,'application/json')}
