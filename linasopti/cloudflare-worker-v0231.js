...
function corsHeaders(request) {
  ...
  return {
    ...
    "Access-Control-Allow-Headers": "Content-Type, X-Lina-Sync-Key",
...
function mergeForwardState(local, incoming) {
  if (!local) return {...validateForwardPackage(incoming),release:"V0.2.31",exportedAt:new Date().toISOString()};
  ...
  return {schema:"LINA-FORWARD-SYNC-1",release:"V0.2.31",exportedAt:new Date().toISOString(),anchor:FORWARD_ANCHOR,tradeEnabled:false,g2,g3};
}
...
async function handleForwardState(request,env) {
  ...
  if(!env.LINA_SYNC_KEY) return json({ok:false,error:"LINA_SYNC_KEY saknas i Worker"},503,request);
  if(request.headers.get("X-Lina-Sync-Key")!==env.LINA_SYNC_KEY) return json({ok:false,error:"Fel synknyckel"},401,request);
      ...
      return json(
        {
          ...
          version: "0.58.8 + forward-state-v0231",
          mode: "paper",
