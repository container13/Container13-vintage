...
function pipelineHtml(es){
  const d=decisionLoad(),pre=String(d?.evidence?.status||'').includes('GITHUB');if(!pre)return '';
  const c=es?.candidate,h=es?.holdoutResults;
  if(h)return `<h2>Candidate → Holdout</h2><div class="forward-lock"><b>🧪 HOLDOUT ${h.status}</b><span>${h.window?.[0]} → ${h.window?.[1]} · ${c?.family||''}</span></div><div class="statusline"><b>Resultat:</b> ${h.metrics?.n||0} affärer · PF ${Number(h.metrics?.pf||0).toFixed(2)} · DD ${(100*Math.abs(Number(h.metrics?.dd)||0)).toFixed(1)} % · P/L ${Number(h.metrics?.pl||0).toFixed(0)}<br><small>Evidens: ${h.evidence?.status||'LOKALT FRYST'}. Ingen omkörning tillåts.</small></div>`;
