...
function mergeForwardState(local, incoming) {
  ...
  const mergeOne=(a,b,kind)=>{
    ...
    let base;
    const cutoff=String(incoming?.completedThrough||"");
    const safeRollback=!!cutoff && ad>cutoff && bd && bd<=cutoff;
    if(safeRollback) {
      base={...b};
      base.history=(b?.history||[]).filter(x=>!x?.marketDate || String(x.marketDate)<=cutoff);
    } else if(bd>ad) base={...b}; else if(ad>bd) base={...a}; else {
    ...
    if(!safeRollback) base.history=mergeHistory(a?.history,b?.history);
    base.lastMarketDate=safeRollback?b?.lastMarketDate:later(a?.lastMarketDate,b?.lastMarketDate);base.lastRefreshAt=later(a?.lastRefreshAt,b?.lastRefreshAt);
  ...
  return {schema:"LINA-FORWARD-SYNC-1",release:"V0.2.43",exportedAt:new Date().toISOString(),anchor:FORWARD_ANCHOR,tradeEnabled:false,completedThrough:incoming.completedThrough||local.completedThrough||null,g2,g3};
}
