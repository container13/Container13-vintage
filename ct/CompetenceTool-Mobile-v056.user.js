// ==UserScript==
// @name         Competence Tool – mobilvy v0.56
// @namespace    container13.mobile.v056
// @version      0.56.0
...
(() => {
  ...
  const styleManning = () => {
    ...
    addStyle('ctm-manning-style', `
      ...
      .topContent .form-group { margin: 0 !important; }
      .topContent .ellipsis { overflow: visible !important; }
      .topContent label, .topContent .controltext { font-size: 13px !important; }
      ...
      #ctm-shift-filter button:first-child { width: 58px !important; }
      ...
      #ctm-view-options {
        ...
        font: 600 14px -apple-system, BlinkMacSystemFont, sans-serif !important;
        position: relative !important;
        top: auto !important;
  ...
  const installMobileSignature = () => {
    ...
    signature.append(
      ...
      Object.assign(document.createElement('span'), { className: 'ctm-signature-product', textContent: 'Competence Tool' }),
  ...
  const installShiftFilter = () => {
    const table = document.querySelector('#shiftScheduleTable');
    if (!table || document.getElementById('ctm-shift-filter')) return false;
  ...
  const installNameOption = () => {
    ...
    checkbox.checked = sessionStorage.getItem('ctm-first-names') !== 'false';
    ...
    const updateNames = () => {
      ...
      sessionStorage.setItem('ctm-first-names', String(checkbox.checked));
    };
