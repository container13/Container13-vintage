...
- Bildytan är försiktigt minskad och Spara anpassning har nu en verklig, synlig egen zon.
- Gulramen är tunnare, instruktionen lugnare och framtidsverktygen tydligare nedtonade.
- Aktiva bildverktyg ger en kort fysisk tryckrespons utan att navigation eller bildfunktion ändrats.
- Rootens `/version.js` är orörd; CCC-versionen finns endast i `/ccc-core/version.js`.

v2.10.127 – modal publiceringsvisning och tydligare bildhierarki
- Kortet Visas på Container13 är helt klickbart och öppnar en centrerad modal i stället för ett utfällt formulär i huvudvyn.
- Modalens val sparas först med Klar. Avbryt, tryck utanför eller Core-Tillbaka stänger utan att ändra publiceringen.
- Använd standard återställer valen i modalens tillfälliga utkast och verkställs först med Klar.
- Anpassa bild visar nu `Objekt X av Y` diskret inne i bildens övre vänstra hörn.
- Den separata räknarraden är borttagen och redigeringskortets guldram är lugnare.
- Spara anpassning har tydligare avstånd från bildverkstaden och Core-footern.
- Frilägg och Bakgrund använder riktiga låssymboler.
- Rootens `/version.js` är orörd; CCC-versionen finns endast i `/ccc-core/version.js`.
...
- Dra, nypzoom och de befintliga explicita zoomkontrollerna fungerar tillsammans med verktygslägena.
- Rotation sparas i bildens separata publiceringsvariant; originalbilden skrivs aldrig över.
- Frilägg och Bakgrund finns som synliga, låsta framtidsplatser så layout och dataflöde kan byggas vidare utan ny omstrukturering.
- Hjälpen för vyn beskriver de nya verktygen och framtidsplatserna.
- Rootens `/version.js` är orörd; CCC-versionen finns endast i `/ccc-core/version.js`.
...
- Ett eller två objekt behåller den centrerade Core-swipens geometri; gul markering ändrar inte kortets mått eller placering.
- När Vision öppnats från Publicera heter footeråtgärden `Klar`; normalt Vision-flöde behåller `Publicera`.

v2.10.113 – kompakt källrad överst i Publicera
- `Foto`, `Bilder` och `Utkast` ligger direkt under modulheadern, före objektområdet.
- Knapparna är lägre med mindre ikoner, horisontellt innehåll och diskretare glöd.

v2.10.112 – tre direkta källknappar i Publicera
- Den extra CCC-popupen är borttagen.
- Publicera visar tre korta, jämnbreda val: `Foto`, `Bilder`, `Utkast`.
- `Foto` öppnar CCC-kameran, `Bilder` öppnar enhetens bildval och `Utkast` använder befintligt utkastflöde.

v2.10.111 – enhetsneutral källtext
- `Ta foto med CCC` fortsätter använda CCC-kameran.
- Det andra valet heter `Välj från enheten` med förklaringen `Bildbibliotek eller filer`.
- v2.10.110 ska hoppas över; dess iPhone-specifika text var inte plattformsneutral.

v2.10.110 – ersatt av v2.10.111

v2.10.109 – flytande källmeny direkt i Publicera
- `Lägg till {singular}` öppnar en kompakt CCC-meny ovanpå den befintliga Publicera-arbetsytan i stället för att navigera till Visions stora källvy.
- `Ta foto med CCC` öppnar Visions kamera direkt via `?mode=publish-add&source=camera` och återställer aktuell grupp/kanal efter Klar eller X.
- `Bildbibliotek / filer` öppnar enhetens säkra bildväljare från samma meny, accepterar flera bilder och skapar Vision-kompatibla lokala objekt direkt i aktuell publiceringsgrupp.
- Menyn stängs med Avbryt, tryck utanför eller Escape. Safari kan efter enhetsvalet visa sin egen systemmeny för Bildbibliotek/Ta bild/Välj filer; detta styrs av iOS.
- v2.10.108:s stora Vision-källvy finns kvar som teknisk fallback men används inte från Publiceras normala Lägg till-knapp. Rootens `/version.js` är orörd.
...
- Publicera sparar aktuell objektgrupp, kanal och verktygsmarkering tillfälligt. `Klar` i kameran sparar de nya Vision-objekten som Publicera-utkast, lägger dem till gruppen och återställer tidigare val.
- Kamerans X återgår till samma Publicera-arbetsyta utan att lägga det aktuella staged-fotot i publiceringsgruppen. Redan autosparade foton bevaras i Vision.
- Vision-starten hålls dold under specialingången så endast CCC-kameran visas. Webbläsarens filkamera används endast som befintlig teknisk fallback om `getUserMedia` saknas eller nekas.
- Rootens `/version.js` är fortsatt orörd.
...
- Varje vald bild skapar ett nytt objekt, sparas i samma lokala Vision-/Publicera-lager och läggs automatiskt till i aktuell publiceringsgrupp.
- Knapptext och status använder Core-terminologins singular/plural.
...
- Objekt centreras när hela raden ryms och går över till fri Core-swipe först vid overflow.
- Kanalradens symmetriska sidpadding låter första och sista kanal visas helt i respektive ändläge.

v2.10.102 – Granska till Vision och bredare slutkontroll
- Granska öppnar markerat objekt i Visions Granska & komplettera och återvänder därefter till samma expresskontroll och verktygsmarkering.
- Objektkorten är något större och linjerar bättre med korten nedanför.
- Kanalikonerna är något större och kanalraden har säker sidluft så vänster kant/glöd inte klipps.

v2.10.101 – grafiska objektverktyg i slutkontrollen
- Markerat objekt får tydligare gul ram/glöd och aktiv statuspill.
- Granska, Anpassa bild och Ta bort är kompakta upphöjda verktygskort med skalbara SVG-ikoner, funktionsfärg och fysisk tryckrespons.

v2.10.101 – grafiska objektverktyg i slutkontrollen
- Markerat objekt får tydligare gul ram/glöd och aktiv statuspill.
- Granska, Anpassa bild och Ta bort är kompakta upphöjda verktygskort med skalbara SVG-ikoner, funktionsfärg och fysisk tryckrespons.

v2.10.100 – enradig slutkontroll med objektverktyg
- Sista kontrollvyn visar alla publiceringsobjekt i en kompakt, fritt swipebar rad.
- Enkeltryck väljer exakt ett objekt för verktygen och markerar det med gul ram; urvalet påverkar inte vilka objekt som publiceras.
- Granska, Anpassa bild och Ta bort är låsta tills ett objekt valts. Ta bort lyfter endast objektet ur aktuell publicering och bevarar det lokala utkastet.
