...
const V0459_CHECKPOINT_KEY='linasopti_signallab2_checkpoint_v0459';
const V0459_CONDS=[
...
async function v0459Share(){const st=document.getElementById('v0459Status');if(!V0459_RESULT){if(st)st.textContent='Ingen Signal Lab 2-rapport finns ännu. Kör labbet först.';return false}const text=v0459Report(),date=new Date().toISOString().slice(0,10),name=`LINAS_OPTI_SIGNAL_LAB_2_ENTRY_B_DISSEKTION_V0459_${date}.txt`;if(!v0451IsIOS()){try{v0451DownloadText(text,name);if(st)st.textContent=`✓ Signal Lab 2-rapport nedladdad: ${name}`;return true}catch(e){if(st)st.textContent='Kunde inte ladda ner Signal Lab 2-rapporten.';return false}}const file=new File([text],name,{type:'text/plain;charset=utf-8'});try{if(navigator.share){if(!navigator.canShare||navigator.canShare({files:[file]})){await navigator.share({title:'Linas Opti Signal Lab 2',files:[file]});return true}await navigator.share({title:'Linas Opti Signal Lab 2',text});return true}}catch(e){if(e?.name==='AbortError')return false}try{v0451DownloadText(text,name);return true}catch{return false}}
...
window.addEventListener('DOMContentLoaded',()=>{document.getElementById('v0458Run')?.addEventListener('click',v0458Run);document.getElementById('v0458Share')?.addEventListener('click',v0458Share);v0458Paint();const jump=document.getElementById('v0413LabJump');if(jump){try{const saved=localStorage.getItem('linasopti_testlab_selected');if(!saved||saved==='v0457Lab')jump.value='v0458Lab'}catch{}}const cp=v0458Load();if(cp&&cp.nextIndex>0){const st=document.getElementById('v0458Status'),bar=document.getElementById('v0458Bar'),months=v0440Months();if(st)st.textContent=`Sparad delkörning ${cp.nextIndex}/${months.length} månader · tryck Fortsätt för att återuppta.`;if(bar)bar.style.width=`${cp.nextIndex/months.length*100}%`;v0458RunLabel(true)}});
